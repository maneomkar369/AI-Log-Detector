...
Edge Server	Python 3.10+, FastAPI, WebSockets, Redis, PostgreSQL, TensorFlow Lite / PyTorch
...
Update SERVER_URL in Config.kt with your ngrok WebSocket URL.

Build and install on your Android device (USB debugging enabled).

Grant required permissions (Usage Stats, Accessibility, Notifications).

5. Test the flow
Tap Start Monitoring – the app connects to the edge server.

Simulate an anomaly: tap Simulate Anomaly (sends 15 rapid app launches).

Within seconds, an alert appears on the device.

Approve neutralization – the edge server logs the action.
...
Update SERVER_URL in the app to wss://your-domain.ngrok.io.

Build release APK and install on target Android devices.
...
Initial baseline: Built from first 7 days of data (mean μ and covariance Σ).

Adaptive update: Exponential moving average with decreasing learning rate.

Drift detection: CUSUM algorithm distinguishes gradual behavior change from sudden anomalies.
